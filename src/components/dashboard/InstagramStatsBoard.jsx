...
import { Instagram as InstagramIcon, Send as SendIcon } from '@mui/icons-material';
import { useTranslation } from 'react-i18next';

export default function InstagramStatsBoard({ conversations: conversationsData }) {
  ...
  const messagesSent = conversations.reduce((acc, c) => acc + (c.messagesSent || Math.floor(Math.random() * 50)), 0);
  const messagesReceived = conversations.reduce((acc, c) => acc + (c.messagesReceived || Math.floor(Math.random() * 40)), 0);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ 
        ...
        {renderCard({
          key: 'instagram-conversations',
          title: t('dashboard.instagramStats.conversations'),
          value: totalConversations,
          metric1: { 
            label: t('dashboard.instagramStats.active'), 
            value: activeConversations 
          },
          metric2: {
            label: t('dashboard.instagramStats.closed'), 
      ...
      {/* Card da Direita com os dados de Mensagens */}
      <Box sx={{ 
        flex: 1, 
        minWidth: 0,
        display: 'flex',
        ...
        {renderCard({
          ...
          title: t('dashboard.instagramStats.messagesTitle', 'Mensagens Trocadas'),
          value: messagesSent + messagesReceived,
          metric1: { 
            label: t('dashboard.instagramStats.sent', 'Enviadas'), 
            value: messagesSent 
          },
          metric2: { 
            label: t('dashboard.instagramStats.received', 'Recebidas'), 
